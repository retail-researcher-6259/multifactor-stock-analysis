...
class StockScoreTrendAnalyzerTechnical:
    ...
    def _plot_forecasting(self, ticker, indices, scores, dates, output_dir):
        ...
        if forecast.get('exp_smoothing') and forecast['exp_smoothing']:
            forecast_indices = list(range(last_index + 1, last_index + len(forecast['exp_smoothing']['forecast']) + 1))
            plt.plot(forecast_indices, forecast['exp_smoothing']['forecast'], 'g-',
                    label='Exp Smoothing', linewidth=2, marker='s')

        if forecast.get('holt') and forecast['holt']:
            forecast_indices = list(range(last_index + 1, last_index + len(forecast['holt']['forecast']) + 1))
            plt.plot(forecast_indices, forecast['holt']['forecast'], 'm-',
    ...
    def _plot_comprehensive_overview(self, ticker, indices, scores, dates, output_dir):
        ...
        if 'macd' in momentum:
            ...
            ax3.plot(indices, momentum['macd_signal'], 'r-', label='Signal', linewidth=1.5)
            ax3.bar(indices, momentum['macd_histogram'], alpha=0.3)
            ax3.legend(loc='best')
        ax3.set_ylabel('MACD')
        ax3.set_title('MACD')
        ...
        bb = self.technical_results.get(ticker, {}).get('bollinger_bands', {})
        if 'percent_b' in bb:
            ax4.plot(indices, bb['percent_b'], 'orange', linewidth=1.5)
        ...
        if forecast and 'arima' in forecast and forecast['arima']:
            ...
            ax6.plot(forecast_indices, forecast['arima']['forecast'][:5], 'r-',
