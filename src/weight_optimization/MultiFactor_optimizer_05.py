...
def main():
    ...
    if api_key == "YOUR_API_KEY_HERE":
        print(" Please set your Marketstack API key in marketstack_config.json")
        return
    ...
    if len(tickers) < 5:
        print(f" Warning: Only {len(tickers)} tickers found. Correlation analysis works best with 10+ tickers.")
    ...
    for ticker in tickers:
        ...
        #         print(f" Insufficient data for {ticker}: only {len(ticker_data)} days")
        # else:
        #     failed_tickers.append(ticker)
    ...
    for i, tk in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist.empty or len(hist) < 200:
                print(f" Insufficient data for {tk}, skipping...")
                continue
