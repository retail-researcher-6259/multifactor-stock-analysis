...
SELECTED_REGIME = r"Strong Bull"  # "Strong Bull", "Steady Growth", or "Crisis/Bear"
...
def fetch_marketstack_data(symbols, date_from, date_to, api_key):
    ...
    if all_symbol_dfs:
        ...
        # Check for duplicate columns (can happen with 300+ tickers)
        if all_price_data.columns.duplicated().any():
            print(f"⚠️ Warning: Found {all_price_data.columns.duplicated().sum()} duplicate columns")
            # Keep only the first occurrence of each column
            all_price_data = all_price_data.loc[:, ~all_price_data.columns.duplicated(keep='first')]
            print(f"✓ Removed duplicates, {len(all_price_data.columns)} columns remaining")
...
def cache_options_oi(ticker, options_chain, date=None):
    ...
    try:
        ...
        print(f"  ⚠️ Failed to cache options OI for {ticker}: {e}")
        return False
...
def load_historical_oi(ticker, lookback_days=30):
    ...
    Returns:
        dict: {date: {strike_type_exp: oi_value}}
    """
    historical_oi = {}
        ...
        if cache_file.exists():
            try:
                with open(cache_file, 'r') as f:
                    ...
                    historical_oi[date] = cached_data.get('data', {})
            except Exception as e:
                continue
...
def main():
    ...
    for ticker in tickers:
        ...
        for base_col in ['Open', 'High', 'Low', 'Close', 'Volume']:
            ...
            if col_name in all_marketstack_data.columns:
                # Fix: Ensure we get a Series, not a DataFrame
                # (handles duplicate column names that can occur with 300+ tickers)
                col_data = all_marketstack_data[col_name]

                # If multiple columns match (duplicate names), take the first
                if isinstance(col_data, pd.DataFrame):
                    col_data = col_data.iloc[:, 0]

                ticker_data[base_col] = col_data
                has_data = True
    ...
    for i, tk in enumerate(tickers, 1):
        ...
        try:
            ...
            raw.append(dict(
                ...
                Liquidity=liq,
                Carry=carry,
                OptionsFlow=options_flow,  # NEW: Institutional options momentum
                Sector=sector,
                Industry=industry,
    ...
    factor_scores = {
        ...
        'carry': [],
        'growth': [],
        'options_flow': []  # NEW: Institutional options momentum
    }
    ...
    for k, rec in df_raw.iterrows():
        ...
        factor_scores['options_flow'].append(rec['OptionsFlow'])  # NEW: Institutional options momentum
        ...
        total = (
                ...
                rec['OptionsFlow'] * WEIGHTS['options_flow']  # NEW: Institutional options momentum
        )
