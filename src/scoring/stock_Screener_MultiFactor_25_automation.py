...
def run_daily_scoring(regime="Steady_Growth", progress_callback=None):
    ...
    today = yesterday.strftime("%m%d")  # e.g. 1015 for Oct 15
    fname = f"top_ranked_stocks_{regime}_{today}.csv"
    output_path = output_dir / fname
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Multifactor Stock Scoring')
    parser.add_argument('regime', nargs='?', default='Steady_Growth',
                        help='Regime name (Steady_Growth, Strong_Bull, Crisis_Bear)')
    parser.add_argument('--ticker-file', type=str,
                        help='Path to ticker file (optional)')
    parser.add_argument('--mode', type=str, default='daily',
                        choices=['daily', 'historical'],
                        help='Scoring mode')
    parser.add_argument('--target-date', type=str,
                        help='Target date for historical mode (YYYY-MM-DD)')

    args = parser.parse_args()

    # If ticker file is provided, update the global TICKER_FILE
    if args.ticker_file:
        import os
        if os.path.exists(args.ticker_file):
            TICKER_FILE = Path(args.ticker_file)
            print(f"Using custom ticker file: {TICKER_FILE}")
        else:
            print(f"Warning: Ticker file not found: {args.ticker_file}")
    ...
    main(regime=args.regime, mode=args.mode, target_date=target_date)