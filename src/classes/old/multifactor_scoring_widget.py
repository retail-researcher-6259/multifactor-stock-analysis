...
class MultifactorScoringThread(QThread):
    ...
    def run(self):
        try:
            ...
            scoring_script_path = PROJECT_ROOT / 'src' / 'scoring' / 'stock_Screener_MultiFactor_25.py'
            ...
            try:
                ...
                import stock_Screener_MultiFactor_25 as scorer
...
class MultifactorScoringWidget(QWidget):
    ...
    def run_scoring_process(self):
        ...
        if mode == "Historical (Range)":
            ...
            # # Calculate number of dates to process
            # num_dates = (end_date - start_date).days + 1
            #
            # # Update UI
            # self.progress_group.setVisible(True)
            # self.progress_bar.setValue(0)
            # self.progress_status.setText(f"Initializing historical analysis for {num_dates} daily records...")
            ...
            # Start historical batch scoring thread
            ...
            # Start specific date scoring thread
            self.scoring_thread = HistoricalSpecificScoringThread(
                target_date, regime_key
            )
...
class HistoricalBatchScoringThread(QThread):
    ...
    def __init__(self, start_date, end_date, interval_days):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            try:
                # Import the module
                import stock_Screener_MultiFactor_25 as screener
...
class HistoricalSpecificScoringThread(QThread):
    ...
    def __init__(self, target_date, regime_key):
        super().__init__()
